...
|------|---------|---------|
| Lexer | 어휘 오류 | `[오류] [라인 N] ...` |
| Parser | 구문 오류 | `[구문 오류] [라인 N] ...` |
| Checker | 의미 오류 | `[의미 오류] [라인 N] ...` |
| Interpreter | 런타임 오류 | `[런타임 오류] [라인 N] ...` |
...
```
print 1 + 2
→ [구문 오류] [라인 1] 값 출력 뒤에 ';'가 필요합니다. ('2' 근처)

x + y = 1;
→ [구문 오류] [라인 1] 잘못된 할당 대상입니다. ('=' 근처)

func () { }
→ [구문 오류] [라인 1] 함수 이름이 필요합니다. ('func' 근처)
```
...
```
{ var a = 1; var a = 2; }
→ [의미 오류] [라인 1] 이미 이 스코프에 같은 이름의 변수가 있습니다. ('a')

{ var a = a; }
→ [의미 오류] [라인 1] 자신의 초기화식에서 지역변수를 읽을 수 없습니다. ('a')

func f(a, a) { }
→ [의미 오류] [라인 1] 파라미터 이름이 중복됩니다. ('a')

return 1;
→ [의미 오류] [라인 1] 함수 외부에서 return을 사용할 수 없습니다.
```
...
```
print notDeclared;
→ [런타임 오류] [라인 1] 미정의된 변수 'notDeclared'.

print 1 + "HI";
→ [런타임 오류] [라인 1] 피연산자는 두 숫자 또는 두 문자열이어야 합니다.

print -true;
→ [런타임 오류] [라인 1] 피연산자는 반드시 숫자여야 합니다.

print 10 / 0;
→ [런타임 오류] [라인 1] 0으로 나눌 수 없습니다.

print 10 % 0;
→ [런타임 오류] [라인 1] 0으로 나눌 수 없습니다.

var x = 1; x();
→ [런타임 오류] [라인 1] 함수가 아닌 대상을 호출했습니다.

func f(a) { } f(1, 2);
→ [런타임 오류] [라인 1] 인자 개수 불일치. 기대: 1, 실제: 2

var arr = Array(3); print arr[5];
→ [런타임 오류] [라인 1] 인덱스 범위를 벗어났습니다. (5)

var arr = Array(3); print arr["x"];
→ [런타임 오류] [라인 1] 인덱스는 반드시 숫자여야 합니다.

var x = 1; print x[0];
→ [런타임 오류] [라인 1] 인덱스 접근은 배열만 지원합니다.

var arr = Array("hi");
→ [런타임 오류] 배열 크기는 숫자여야 합니다.

var arr = Array(-1);
→ [런타임 오류] 배열 크기는 0 이상이어야 합니다.

var arr = Array(2000000);
→ [런타임 오류] 배열 크기가 너무 큽니다.
```
